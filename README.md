...
**AI Q&A (local):** `POST /api/ask` — a LangChain/`create_agent` ReAct loop on LangGraph (Claude) that calls those three modes as tools and answers with citations. Requires `ANTHROPIC_API_KEY` in `backend/.env` (see `backend/.env.example`).  
