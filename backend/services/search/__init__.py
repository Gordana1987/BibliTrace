...
from services.search.common import active_corpora_from_request, default_limit, page_hits
...
def search(request: SearchRequest) -> SearchResponse:
    ...
    for corpus in corpora:
        try:
            results[corpus] = fn(term, corpus, offset=offset, limit=limit)
    ...
    if request.mode == "lemma":
        ...
        msg = "Семантичка претрага још није укључена (чисти dense у припреми)."
    else:
        total_any = sum(r.total for r in results.values())
        msg = (
